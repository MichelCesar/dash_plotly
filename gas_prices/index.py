...
'PREÇO MÍNIMO REVENDA', 'PREÇO MÁXIMO REVENDA', 'DESVIO PADRÃO REVENDA', 
            ...
            dbc.Col([
                dbc.Card([
                    dbc.CardBody([
                        ...
                        html.H6('Comparação tempora entre estados'),
                        dbc.Row([
                            dbc.Col([
                                dcc.Dropdown(
                                    id='select_estado0',
...
def animation(data, estado, toogle):
    template = template_theme1 if toogle else template_theme2

    dff = pd.DataFrame(data)
    mask = dff.ESTADO.isin(estado)
